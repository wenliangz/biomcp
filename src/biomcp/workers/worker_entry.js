...
const json = (o, s = 200) => new Response(JSON.stringify(o, null, 2), { status: s, headers: { 'Content-Type': 'application/json', ...CORS } });
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (url.pathname === '/status' || url.pathname === '/debug') return json({ worker: 'BioMCP-authless', remote: REMOTE_MCP_SERVER_URL, forwardPath, resourceEndpoint });
    if (url.pathname === '/sse' || url.pathname === '/events') return serveSSE(req, ctx);
...
async function proxyPost(req, path, sid) {
  ...
  const target = `${REMOTE_MCP_SERVER_URL}${path}?session_id=${encodeURIComponent(sid)}`;
  ...
  try {
    ...
    const timeout = jsonBody.method && 
                   (jsonBody.method === 'tools/list' || 
                    jsonBody.method === 'resources/list') ? 
                   30000 : 10000;
