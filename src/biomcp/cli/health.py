...
from .. import const, http_client, logger
...
async def check_api_endpoint(url: str, name: str, params: dict = None, method: str = "GET") -> Dict:
    """Check if an API endpoint is accessible and responding."""
    try:
        status, content = await http_client.call_http(method, url, params or {})
        return {
            "name": name,
            "url": url,
            "status": status,
            ...
            "content": content[:500] if len(content) > 500 else content  # Truncate long responses
        }
...
async def check_all_api_endpoints() -> List[Dict]:
    ...
    endpoints = [
        ...
        },
        {
            ...
            "params": {"query": "BRAF", "concepts": "gene", "page": 1, "size": 1, "text": "@CHEMICAL_remdesivir"}
        },
    ...
    tasks = [check_api_endpoint(endpoint["url"], endpoint["name"], endpoint["params"]) for endpoint in endpoints]
...
def check_network_connectivity() -> Dict:
    ...
    try:
        ...
        return {"status": "Connected", "message": "Internet connection is available"}
    except OSError:
        return {"status": "Disconnected", "message": "No internet connection detected"}
...
def display_api_health(results: List[Dict], verbose: bool = False) -> None:
    ...
    for result in results:
        status_style = "green" if result["accessible"] else "red"
        table.add_row(
            result["name"],
            result["url"],
            f"{result['status']}", 
    ...
    if verbose:
        for result in results:
            if not result["accessible"]:
                ...
                console.print(Panel(result["content"], title=f"{result['name']} Response", border_style="red"))


def display_system_health(system_info: Dict, network_info: Dict, env_info: Dict) -> None:
    ...
    if "error" in system_info:
        ...
        resource_table.add_row("Memory Total", f"{system_info['memory']['total']:.2f} GB")
        resource_table.add_row("Memory Available", f"{system_info['memory']['available']:.2f} GB")
        resource_table.add_row(
            "Memory Usage", 
            f"{system_info['memory']['percent_used']}%",
            style="green" if system_info['memory']['percent_used'] < 90 else "red"
        )
        resource_table.add_row("Disk Total", f"{system_info['disk']['total']:.2f} GB")
        resource_table.add_row("Disk Free", f"{system_info['disk']['free']:.2f} GB")
        resource_table.add_row(
            "Disk Usage", 
            f"{system_info['disk']['percent_used']}%",
            style="green" if system_info['disk']['percent_used'] < 90 else "red"
        )
    ...
    env_table.add_row("Network", network_info["status"], 
    ...
    env_table.add_row("Psutil Version", env_info["psutil_version"], 
...
@health_app.command()
def check(
    api_only: bool = typer.Option(False, "--api-only", help="Check only API endpoints"),
    system_only: bool = typer.Option(False, "--system-only", help="Check only system health"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show detailed error information and API responses"),
):
    """
    ...
    if not api_only and not system_only:
        ...
        if "error" in system_info:
            ...
            system_health = (
                network_info["status"] == "Connected" and
                system_info["memory"]["percent_used"] < 90 and
                system_info["disk"]["percent_used"] < 90
            )
        ...
        if api_health and system_health:
            ...
            if verbose:
                console.print("[yellow]Run with --verbose flag to see detailed error information[/yellow]")
