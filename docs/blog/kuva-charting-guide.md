...
For multi-gene co-occurrence, `--chart heatmap` renders the full NxN both-mutated matrix. For mutation comparisons, `--chart stacked-bar` shows mutated versus not-mutated sample counts across groups while `--chart bar` continues to show mutation rate.
...
BioMCP currently exposes 10 study-chart modes on top of Kuva's 29 plot types. Scatter plots for two-gene expression comparisons and waterfall plots for mutation burden are still on the roadmap.
