...
[Kuva](https://github.com/Psy-Fer/kuva) (v0.1.4) is the charting engine behind BioMCP's `--chart` flag. BioMCP currently exposes 10 chart modes (`bar`, `stacked-bar`, `pie`, `heatmap`, `histogram`, `density`, `box`, `violin`, `ridgeline`, `survival`) backed by Kuva plots and rendered to terminal, SVG, and PNG.
